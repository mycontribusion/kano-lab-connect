...
export default function LabDetail({ lab, requestedTests, userLocation, onClose }: LabDetailProps) {
    ...
    return (
        ...
        >
            {/* Header */}
            <div className={cn(
                'p-5 border-b relative transition-all duration-300',
                showDirections ? 'bg-primary/5' : (lab.featured ? 'bg-gradient-to-br from-amber-50 to-white' : 'bg-muted/30')
            )}>
                <button
                    onClick={onClose}
                ...
                <div className="pr-8">
                    <div className="flex items-center gap-2 mb-1">
                        {showDirections ? (
                            <span className="text-[10px] font-black uppercase tracking-widest bg-primary/10 text-primary border border-primary/20 rounded-full px-2 py-0.5 flex items-center gap-1">
                                <Navigation2 className="w-3 h-3" /> Navigation Mode
                            </span>
                        ) : (
                            <>
                                {lab.featured && (
            ...
            <div className="flex-1 overflow-hidden relative">
                <AnimatePresence mode="wait">
                    {showDirections && userLocation ? (
                        ...
                        >
                            <DirectionsMap userLocation={userLocation} lab={lab} />
                        </motion.div>
                    ) : (
                        <motion.div
                            key="info"
            ...
            <div className="p-4 border-t bg-background/80 backdrop-blur flex gap-2">
                {showDirections ? (
                    <>
                        ...
                        <Button
                            className="h-11 font-bold text-sm gap-2 flex-1 bg-blue-600 hover:bg-blue-700 text-white"
                    ...
                    <>
                        ...
                        <Button
                            className="h-11 font-bold text-sm gap-2 flex-1 bg-green-600 hover:bg-green-700 text-white disabled:opacity-50"
                            disabled={!userLocation}
                            onClick={() => setShowDirections(true)}
                            title={!userLocation ? 'Location access required for in-app directions' : 'View directions in app'}
                        >
                            <Navigation2 className="w-4 h-4" />
                            {userLocation ? 'Get Directions' : 'Enable Location'}
                        </Button>
                    </>
                )}
            </div>
